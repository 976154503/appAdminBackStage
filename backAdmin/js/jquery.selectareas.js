...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
			...
			// F. TOST
			resizeArea = function (width, height) {				
				area.width  = width;
				area.height = height;
				refresh();
            },
            ...
            //热点信息初始化位置
            updateBtShowInformation = function (visible) {
                if ($btShowInformation) {
                    $btShowInformation.css({
                        display: (visible & area.visibility) ? "block" : "none",
                        left: area.x + area.width - 20,
                        top: area.y - $btShowInformation.outerHeight() - 5,
                        "z-index": area.z + 1
                    });
                }
            },
        ...
        // Initialize all handlers 初始化所有操作
        if (options.allowResize) {
        ...
        // initialize delete button 初始化删除
        ...
        // 初始化热点信息
        if (options.allowShowInformation) {
            var bindToShowInformation = function ($obj) {
                //web手机端，clikc会有200-300ms的延时，所以要用tap代替click作为点击事件
                $obj.click(showInformation)
                    .bind("touchstart", showInformation)
                    .bind("tap", showInformation);
                return $obj;
            };
            $btShowInformation = bindToShowInformation($("<div class=\"show-area-information\" />"))
              .append(bindToShowInformation($("<div class=\"select-areas-information\" />")))
              .insertAfter($selection);
        }
		// F. TOST
		$tag = $("<div class=\"tag-area\">---</div>")                
                .insertAfter($selection);
    ...
    $.imageSelectAreas.prototype.init = function (object, customOptions) {
        ...
        //给this.$holder的div一个外层盒子div,存为变量this.$image
        this.$image.wrap(this.$holder)
            .css({
                position : "absolute"
            });

        //在每一个this.$image元素之后插入一个class名字为select-areas-overlay的元素，存在变量this.$overlay
        ...
        //在每一个this.$overlay元素之后插入一个div元素，存在变量this.$trigger
        this.$trigger = $("<div />")
            .css({
	...
	// F. TOST	
	$.imageSelectAreas.prototype.setTag = function (id, new_tag) {        
	...
	$.imageSelectAreas.prototype.resizeArea = function (id, width, height) {        
		...
		this._areas[id].resizeArea(width, height);
    };
		...
		{ // Method call			
            ...
            this.each(function() {
                ...
                image.onload = function() {		
					// console.log("INFO: " + "selectarea.js $.fn.selectAreas image.onload = function()");				
